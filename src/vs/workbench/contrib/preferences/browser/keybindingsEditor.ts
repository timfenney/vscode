...
export class KeybindingsEditor extends EditorPane implements IKeybindingsEditorPane {
	...
	async copyKeybindingCommandName(keybinding: IKeybindingItemEntry): Promise<void> {
		this.selectEntry(keybinding);
		this.reportKeybindingAction(KEYBINDINGS_EDITOR_COMMAND_COPY_COMMAND_NAME, keybinding.keybindingItem.command, keybinding.keybindingItem.keybinding);
		await this.clipboardService.writeText(keybinding.keybindingItem.command);
	}
		...
		if (e.element.templateId === KEYBINDING_ENTRY_TEMPLATE_ID) {
			...
			this.contextMenuService.showContextMenu({
				...
				getActions: () => [
					...
					this.createCopyCommandNameAction(keybindingItemEntry),
					new Separator(),
	...
	private createCopyCommandNameAction(keybinding: IKeybindingItemEntry): IAction {
		return <IAction>{
			label: localize('copyCommandName', "Copy Command Name"),
			enabled: true,
			id: KEYBINDINGS_EDITOR_COMMAND_COPY_COMMAND_NAME,
			run: () => {
				console.log('yodawg', keybinding);
				this.copyKeybindingCommandName(keybinding);
			}
		};
	}
